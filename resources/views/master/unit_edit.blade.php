    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
              <h3 class="card-title">Edit data Unit RS PKU Muhammadiyah Sekapuk</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                @if ($message = Session::get('success'))
                    ...
                    @endif
                <form action="{{ route('unit_update', $id) }}" method="post">
                    @csrf
                    @method('put')
                    <div class="form-group">
                        <label>Kode Unit</label>
                        <input type="text" name="kode_unit" class="form-control" value="{{ old('kode_unit',$kode_unit) }}">
                        @error('kode_unit')
                            {{ $message }}
                        @enderror
                      </div>
                      <div class="form-group">
                        ...
                        <input type="text" name="nama_unit" class="form-control" value="{{ old('nama_unit',$nama_unit) }}" required>
                      </div>
